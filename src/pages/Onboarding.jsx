  ...
  return (
    <div className="min-h-dvh p-4 flex flex-col bg-[var(--bg)] justify-between max-w-md mx-auto md:max-w-2xl md:justify-center">
      ...
      <main className="flex-1 flex flex-col justify-center items-center my-auto">
        <div className="w-64 h-64 flex items-center justify-center overflow-hidden mb-6 md:mb-0 md:w-3/2">
          <img
            src={currentScreen.image}
            alt={currentScreen.title}
            className="w-full h-full object-contain transition-all duration-300 transform object-top scale-100"
          />
        </div>
      ...
      <footer className="w-full mt-6">
        <StepIndicators total={onboardingData.length} current={currentIndex} />
        <button
          type="button"
          onClick={handleNext}
          className="w-full bg-brand hover:bg-brand-dark text-white font-medium text-sm py-3 px-4 mt-4 rounded-lg shadow-sm transition-colors focus:outline-none focus:ring-2 focus:ring-brand focus:ring-offset-2"
        >
